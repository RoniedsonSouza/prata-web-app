...
public sealed class FakePaymentGateway : IPaymentGateway
{
    ...
    private readonly List<SettlementLine> _extrato = [];

    public Task<Result<PayoutAccountRef>> CriarRecebedorAsync(RecebedorRequest request, CancellationToken cancellationToken)
    {
        _ = cancellationToken;
    ...
    public Task<Result<KycStatusDto>> ConsultarKycAsync(PayoutAccountRef reference, CancellationToken cancellationToken)
    {
        _ = reference;
        _ = cancellationToken;
    ...
    public Task<Result<ChargeRef>> CriarCobrancaAsync(ChargeRequest request, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(request.CreditCardToken) && LooksLikePan(request.CreditCardToken))
            return Task.FromResult(
                Result.Failure<ChargeRef>(Error.Validation("CARTAO_PROIBIDO", "Payload nao pode conter PAN/CVV (RN-FIN-002)."))
            );
    ...
    public Task<Result<ChargeState>> ConsultarCobrancaAsync(ChargeRef reference, CancellationToken cancellationToken)
    {
        _ = cancellationToken;
        if (!_charges.TryGetValue(reference.ExternalChargeId, out var state))
            return Task.FromResult(Result.Failure<ChargeState>(Error.Validation("COBRANCA_NAO_ENCONTRADA", "Cobranca ausente no fake.")));
    ...
    public Task<Result<RefundRef>> EstornarAsync(ChargeRef reference, Money? parcial, CancellationToken cancellationToken)
    {
        _ = reference;
        _ = parcial;
        _ = cancellationToken;
    ...
    public Result<WebhookEnvelope> VerificarEAnalisar(string rawBody, IReadOnlyDictionary<string, string> headers)
    {
        ...
        var envelope = new WebhookEnvelope(parts[0], parts[1], Guid.Parse(parts[2]), parts[3], EnsureChargeIdInPayload(parts[4], parts[3]));
    ...
    public Task<Result<IReadOnlyList<SettlementLine>>> ObterExtratoAsync(DateOnly de, DateOnly ate, CancellationToken cancellationToken)
    {
        _ = de;
        _ = ate;
        _ = cancellationToken;
        IReadOnlyList<SettlementLine> lines = _extrato.ToList();
        return Task.FromResult(Result.Success(lines));
    }

    public void SimularConfirmacao(string chargeId, DateTimeOffset at) => _charges[chargeId] = new ChargeState("Confirmado", at);

    /// <summary>Semeia linha de extrato para testes de conciliacao (RN-FIN-050).</summary>
    public void SemearExtrato(SettlementLine line) => _extrato.Add(line);

    private static string EnsureChargeIdInPayload(string payload, string chargeId)
    {
        if (string.IsNullOrWhiteSpace(payload) || payload == "{}")
            return $"{{\"chargeId\":\"{chargeId}\"}}";

        if (payload.Contains("chargeId", StringComparison.Ordinal))
            return payload;

        return $"{{\"chargeId\":\"{chargeId}\",\"raw\":{System.Text.Json.JsonSerializer.Serialize(payload)}}}";
    }
